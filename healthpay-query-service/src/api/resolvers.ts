...
const Mutation = {
  ...
  merchantLogin: async (_: any, { email, password }: { email: string; password: string }) => {
  ...
  topUpWallet: async (_: any, { walletId, amount, method }: { walletId: string; amount: number; method: string }, context: Context) => {
    ...
    // Admin Mutations
    adminCreateUser: async (_: any, { input }: any) => {
      const { fullName, phone, email, initialBalance } = input;
    ...
    adminUpdateUser: async (_: any, { input }: any, context: any) => {
      const { id, ...data } = input;
    ...
    adminCreateMerchant: async (_: any, { input }: any) => {
      const { businessName, phone, email } = input;
      const merchantId = `MRC-${String(Date.now()).slice(-6)}`;
      const bcrypt = require("bcryptjs");
      const defaultPassword = await bcrypt.hash("Merchant@123", 10);
    ...
    adminUpdateMerchant: async (_: any, { input }: any, context: any) => {
      const { id, ...data } = input;
